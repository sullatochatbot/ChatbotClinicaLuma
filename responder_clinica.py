...
# ===== Persistência via WebApp (AGORA com rota configurável) ==================
def _post_webapp(payload: dict, rota: str = "captacao") -> dict:
    """
    Envia JSON para o WebApp.
    rota: 'captacao' (marketing_captacao) ou 'chatbot' (captação_chatbot)
    - Usa CLINICA_SHEETS_URL (completa, fim /exec)
    - Garante query ?route=<rota>
    - Inclui secret (se existir)
    ...
    if "route=" in base:
        url = re.sub(r"route=[^&]*", f"route={rota}", base)
    else:
        url = base + ("&" if "?" in base else "?") + f"route={rota}"

    headers = {"Content-Type": "application/json"}
    try:
        print(f"[SHEETS->] url:{url} | rota:{rota}")
...
def _map_to_captacao(d: dict) -> dict:
    ...
    payload = {
        ...
        "convenio": convenio,               # nome do convênio
...
def _add_pesquisa(ss, d):
    ...
    # Pesquisa interessa ao marketing – envia só para captacao
    _post_webapp(_map_to_captacao(dd), rota="captacao")
...
def _validate(key, v, *, data=None):
    v = (v or "").strip()
    if key == "cpf" and len(_cpf_clean(v)) != 11:
        return "CPF inválido."
    if key == "nasc" and not _date_ok(v):
        return "Data inválida. Use o formato dd/mm/aaaa."
    if key == "convenio" and (data or {}).get("forma") == "Convênio" and not v:
        return "Informe o convênio."
    if key == "cep" and not _cep_ok(v):
        return "CEP inválido (8 dígitos)."
    if key == "numero" and not v:
...
def _fields_for(route, d):
    if route == "consulta":
        return _comuns_consulta(d)
    if route == "exames":
        return _comuns_exames(d)
    if route == "editar_endereco":
        return [("cep", "Informe seu CEP:"), ("numero", "Informe o número:")]
    if route == "retorno":
    ...
    if mtype == "text":
        ...
        if ses and ses.get("stage") == "origem_menu":
            ...
            if not escolha:
                ...
                _send_text(wa_to, _origem_menu_texto())
                return
            op = int(escolha)
            if op == 0:
            ...
            if op == 5:  # Outros (aberto)
                ...
                _send_text(wa_to, "Pode nos dizer em poucas palavras de onde nos conheceu?")
                return
...
def _finaliza_ou_pergunta_proximo(ss, wa_to, ses):
    ...
    if not pend and route in {"consulta", "exames"} and not data.get("_confirmado"):
        ...
        ses["stage"] = "confirmar"; SESS[wa_to] = ses
        return
...
def _continue_form(ss, wa_to, ses, user_text):
    ...
    if (route == "consulta" and stage == "especialidade"):
        _ask_especialidade_num(wa_to, ses)
        return
    if (route == "exames" and stage == "exame_num"):
        _ask_exame_num(wa_to, ses)
        return
